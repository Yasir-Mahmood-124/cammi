...
const Icp: React.FC<IcpProps> = ({
  ...
  const storedProject = typeof window !== "undefined" ? localStorage.getItem("currentProject") : null;
  ...
  useEffect(() => {
    if (selection === "no" && unansweredData && workflowState === 'user-input') {
    ...
    >
      ...
      {workflowState === 'user-input' && (
        ...
        >
          ...
          {selection === "no" && unansweredLoading && renderLoading("Loading unanswered questions...")}
          {selection === "no" && unansweredError && renderError("Failed to load questions.")}
